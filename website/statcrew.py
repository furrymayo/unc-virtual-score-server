...
def _parse_statcrew_xml(xml_text):
    ...
    if is_baseball or visitor_team or home_team:
        ...
        # Find current pitcher (the one with most recent appearance / last in list)
        # In StatCrew, pitchers appear in order of appearance
        for vh in ["V", "H"]:
            if vh in parsed.get("pitchers", {}):
                pitchers = parsed["pitchers"][vh]
                if pitchers:
                    # Last pitcher in the list is typically current
